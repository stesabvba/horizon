...
$content.="<form id='' class='form-horizontal' role='form' data-toggle='validator' method='POST'>";
	...
	$content.="<div class='panel-heading'>" . ucfirst(translate('mailinglist_message_edit')) . "</div>";
	$content.="<div class='panel-body'>";
	$content.="<input type='hidden' name='form_action' value='mailinglist_message_edit'>";
	...
	$content.="<input name='name' id='name' type='text' class='form-control' value='". $mailinglist_message->name . "' required/>";
	$content.="</div>";
	$content.="</div>";
	//name
	$content.="<div class='form-group'>";
	...
	foreach($languages as $language){
		...
		$mmd = $mailinglist_message->details->where('language_id',$language->id)->first();
		
		if($mmd!=null){
			$subject = $mmd->subject;
			$message_content = $mmd->message;
		}else{
			$subject = "";
			$message_content = "";
		}
		...
		$content.="<div>";
			...
			$content.="<input type='text' name='subject[". $language->id . "]' value='" . $subject ."' class='form-control' required>";
		$content.="</div>";
		$content.="<div>";
			$content.="<label for='content' class='form-control-label'>" . ucfirst(translate('content')) . " " . $language->shortname . ":</label>";
		$content.="<textarea name='content[" . $language->id . "]' id='content_". $language->id . "'  class='wysiwyg'>$message_content</textarea>";
		$content.="</div>";
		$content.="</div>";
